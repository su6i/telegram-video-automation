...
📚 **Documentation:** [CLI Usage](docs/CLI.md) | [Architecture](docs/ARCHITECTURE.md) | [Channel Layout](docs/CHANNEL-LAYOUT.md) | [Knowledge Pipeline](docs/KNOWLEDGE-PIPELINE.md)
...
> **`--run-now` is what writes.** Without it the indexer prints what it would
> update and touches nothing. The placeholder-edit path used to ignore the flag
> and rewrite the live index on a "dry run".

> **One entry per lesson, under its own course.** A re-uploaded lesson leaves
> two video messages in the channel; the newest wins and the older is dropped.
> The course and section headings are repeated with `(cont.)` whenever a
> heading's lessons spill into the next post.

> **Where the lesson number comes from.** The channel carries two caption
> generations: the early uploads open with `NNN - Title`, the later ones put the
> course and the section on the first two lines and the index only on the third.
> `src/caption_index.py` reads the index off whichever header line carries it —
> matching only the first line printed `???` instead of a number and dropped the
> section grouping for every newer lesson.
...
`.agent/constitution` and `.agent/skills` are symlinks into the shared
`agent-constitution` repository — the single source of truth (rule 045). They
used to be 343 copied files that had drifted from the originals; never copy
them back in, update them in `agent-constitution` instead.

## 🔐 What never lives in this repository

The site being scraped and the channel being published to are private data. The
repository is generic: it names no site, no course and no channel.

| Data | Where it lives |
| --- | --- |
| API keys, bot token, channel id, site URLs | `.env` (git-ignored; keep the real file in the vault) — see `.env.example` |
| Site identity: product slug → course display name | `site_profile.json` outside the tree, found via `SITE_PROFILE` or `.storage/site_profile.json` (a symlink into the vault) |
| Scraped lesson text, channel caption dumps, cookies, session files | vault only — `backup_captions.json`, `scraped_content.json`, `course_structure.json`, `*_cookies.json`, `*.session` are all ignored |

Adding a hardcoded site name, course name or channel name to a tracked file is
a defect, not a shortcut: use `src/site_profile.py`.
