...
class PrimaryScraper(BaseScraper):
    ...
    def get_lesson_details(self, url):
        ...
        if section_title == "General":
            # Common in course-platform themes: h5.panel__sub-title > a
        ...
        if course_title == "Unknown Course":
             # 1. Product Link (theme selector: a link into /products/<slug>)
        ...
        # Strategy D: URL Regex Fallback — /products/<slug>/...
        if course_title == "Unknown Course":
            import re
            match = re.search(r'/products/([^/]+)', url)
            if match:
                slug = match.group(1)
                # The display name of a slug is site-specific data: it comes
                # from the runtime profile, never from this file (rule 035).
                course_title = course_title_for(slug, slug.replace("-", " ").title())
    ...
    def _extract_course_title(self, driver, soup):
        ...
        # Page titles are site-specific: mapped through the runtime profile.
        return course_title_for(course_title, course_title)
