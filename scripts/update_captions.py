...
async def resolve_channel(app: Client) -> Tuple[Optional[int], Optional[str]]:
    ...
    if CHANNEL_ID_ENV:
        ...
        try:
            ...
            # A freshly created session has an empty peer cache, so resolving a
            # channel by raw id fails with "Peer id invalid" until the dialog
            # list has been walked once. Prime the cache and retry the SAME id.
            print(f"⚠️ CHANNEL_ID not resolvable yet ({e}). Priming peer cache...")
            found = False
            try:
                ...
                print(f"⚠️ Dialog scan failed: {scan_err}")
            ...
            if found:
                try:
                    ...
                    print(f"⚠️ CHANNEL_ID still unresolvable: {retry_err}")

            # CHANNEL_ID was configured explicitly: never silently retarget
            # another channel from this account.
            print(f"❌ CHANNEL_ID {cid} is configured but unreachable from this account.")
            print("   Refusing to guess a different channel. Fix CHANNEL_ID in .env,")
            print("   or make sure this account is a member of that channel.")
            return None, None
    ...
    # 4) fallback
    # Reached only when NO channel was configured at all (no CHANNEL_ID,
    # no INVITE_LINK, no CHANNEL_USERNAME). Guessing is acceptable only here.
    print("🔍 No channel configured. Searching your account dialogs...")
    try:
        best = None
        async for d in app.get_dialogs():
...
async def create_index_posts(app: Client, chat_id: int, planned, title="📚 Video Index", per_post_limit=4090, start_offset=0, dry_run=True):
    ...
    # The channel holds several courses, so the post header stays generic and
    # the course is named in the body wherever it changes. Naming the first
    # lesson's course at the top of every post labelled six of the seven posts
    # with a course most of their lessons do not belong to.
    courses = {item.get("course") for item in planned_sorted if item.get("course")}
    header = f"{LRM}<b>{title}</b>\n"
    if len(courses) == 1:
    ...
    def format_line(item):
        ...
        # Determine number and title from whichever header line carries the
        # index — the course and section lines come first in newer captions.
        num, caption_title = extract_index_and_title(newc)
        ...
        if display_name:
            ...
            title_txt = caption_title or newc.split('\n')[0].strip()
    ...
    async def flush():
        nonlocal chunk_lines, chunk_len, placeholder_idx, overflowed
        if not chunk_lines:
            return True
        chunk = header + "\n".join(chunk_lines)
        ...
        if target_mid:
            try:
                ...
                # A dry run must not touch the channel. This branch used to
                # edit regardless, so --only-index without --run-now rewrote
                # the live index while claiming to be a dry run.
                if dry_run:
                    print(f"   📝 Would update Index Post #{target_mid} "
                          f"({placeholder_idx + 1}/{len(placeholders)}, {len(chunk)} chars)")
                    placeholder_idx += 1
                    chunk_lines = []
                    chunk_len = len(header)
                    return True
    ...
    def add(text_line):
        nonlocal chunk_len
        chunk_len += len(text_line) + (1 if chunk_lines else 0)
        chunk_lines.append(text_line)

    def course_header(name, continued=False):
        return f"\n{LRM}<b>🎓 {name}{' (cont.)' if continued else ''}</b>"

    def section_header(name, continued=False):
        return f"\n{LRM}<b>📁 {name}{' (cont.)' if continued else ''}</b>"
    ...
    for item in planned_sorted:
        ...
        if course and course != last_course and len(courses) > 1:
            head = course_header(course)
            if chunk_len + len(head) > per_post_limit:
        ...
        if chunk_len + len(line) + (1 if chunk_lines else 0) > per_post_limit:
            if not await flush():
                break
            # A post that continues a section has to say which one, otherwise
            # every entry after a chunk break reads as belonging to whatever
            # heading happened to be at the top of the previous post.
            if last_course and len(courses) > 1:
                add(course_header(last_course, continued=True))
            if last_section:
                add(section_header(last_section, continued=True))
        add(line)

    # Final Buffer
    await flush()

    # OVERFLOW WARNING — only when a chunk actually had no placeholder left.
    # Consuming every placeholder is not an overflow: the last run fit all 283
    # lessons into its 7 posts and still printed this.
    if overflowed:
        print("\n" + "!"*60)
        print("⚠️  WARNING: INDEX OVERFLOW DETECTED!")
        print(f"⚠️  {overflowed} index post(s) had no reserved placeholder left.")
        print(f"⚠️  {placeholder_idx} placeholder(s) were used.")
        print("👉  Fix: Create more blank messages in the channel and run again.")
        print("!"*60 + "\n")

def dedupe_by_lesson(videos):
    """One video per lesson number — the newest upload wins.

    A re-uploaded lesson leaves both copies in the channel (354 video
    messages for 283 lessons), and listing both puts the same lesson in the
    index twice. The higher message id is the later upload, which is the one
    the message map points at too.
    """
    best = {}
    for v in videos:
        ...
        if current is None or v["message_id"] > current["message_id"]:
            best[num] = v
    dropped = len(videos) - len(best)
    if dropped:
        print(f"🧹 {dropped} duplicate/unnumbered video message(s) dropped from the index")
