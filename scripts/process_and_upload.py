...
# Telegram hard limit for a plain text message is 4096 characters.
# We keep a safe margin for the "Continued (n/m)" header and markdown entities.
TEXT_MESSAGE_LIMIT = 4096
TEXT_CHUNK_SIZE = 3800


def split_text_into_chunks(text, limit=TEXT_CHUNK_SIZE):
    """
    Split a long description into Telegram-sized chunks.
    Breaks on paragraph, then line, then sentence, then word boundaries so
    that no content is ever dropped.
    """
    text = (text or "").strip()
    if not text:
        return []
    ...
    while len(text) > limit:
        ...
        if cut < limit * 0.5:
            cut = window.rfind(' ')
        if cut <= 0:
            cut = limit  # no boundary at all: hard cut, still lossless
...
async def send_overflow_messages(app, bot, overflow_text, reply_to_id):
    """
    Send the part of the description that did not fit in the video caption,
    immediately after the video, as one or more replies.

    Every chunk replies to the previous message so the whole description stays
    contiguous under the video. Returns the number of messages sent.
    """
    chunks = split_text_into_chunks(overflow_text)
    if not chunks:
        return 0
    ...
    for n, chunk in enumerate(chunks, 1):
        header = "\U0001F4C4 **Continued:**" if total == 1 else f"\U0001F4C4 **Continued ({n}/{total}):**"
        body = f"{header}\n\n{chunk}"

        try:
            if app is not None and app.is_connected:
                ...
                parent_id = getattr(msg, 'id', parent_id)
                ...
                parent_id = getattr(msg, 'message_id', parent_id)
            else:
                print("   \u26a0\ufe0f No transport available to send overflow.")
                break
            sent += 1
            print(f"   \u2705 Overflow part {n}/{total} sent.")
            await asyncio.sleep(1)
        except Exception as exc:
            print(f"   \u26a0\ufe0f Failed to send overflow part {n}/{total}: {exc}")
            break
...
async def main():
    ...
    app = None
    if has_user_creds:
        app = Client("hybrid_account", api_id=api_id, api_hash=api_hash, workdir=root_dir)
    ...
    try:
        ...
        if is_first_upload and args.dry_run:
            print("🆕 First run detected, but this is a dry run: NOT reserving index placeholders.")
        elif is_first_upload:
        ...
        for i, m_video in enumerate(manifest_videos, 1):
            ...
            # Load Rich Metadata and build the exact caption that will be sent.
            # scripts/preview_captions.py renders the same call offline.
            meta = load_video_metadata(filename)
            extra = load_extra_content(meta['url']) if meta else None
            caption, overflow_text = build_caption(meta, extra, title)
            need_overflow = bool(overflow_text)
            ...
            if first_msg and need_overflow and overflow_text:
                print("   📄 Description split. Sending remainder right after the video...")
                ...
                if reply_id:
                    await send_overflow_messages(
                        app if has_user_creds else None,
                        bot if bot_available else None,
                        overflow_text,
                        reply_id,
                    )
                else:
        ...
        # Trigger Indexing
        if args.dry_run:
            print("\n🧾 Dry run: skipping the Index Post update (it talks to Telegram).")
            return

        print("\n🧾 Triggering automatic Index Post update...")
        try:
            cmd = [sys.executable, os.path.join(root_dir, "scripts", "update_captions.py"), "--run-now", "--only-index"]
